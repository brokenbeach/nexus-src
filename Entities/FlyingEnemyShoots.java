...
public class FlyingEnemyShoots extends FlyingEnemy {
	
	Vector2D health;
	double wave = 0.0;
	boolean up = true;
	
	public FlyingEnemyShoots( LevelState ls, double x, double y, double dx, double dy, double ddx, double ddy, TileMap tileMap) {
		super(ls, x, y, dx, dy, ddx, ddy, tileMap);
		image = SpriteStore.get().getImage("flyingenemy1");
		hitImage = SpriteStore.get().getImage("flyingenemy1hit");
	}
	...
	private void fire() {
		...
		if( (int) (Math.random() * rand) == 0 ) {
			...
			ls.enemyBullets.add(new EnemyBullet(ls, pos.x, pos.y, dx, dy, 0, 0, rotation, tileMap));
		}
	}
